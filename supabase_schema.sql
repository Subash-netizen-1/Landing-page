...
create policy "Allow attendees write to all roles (Super Admin, Manager, Staff)"
  on public.attendees for all
  to authenticated
  using (true); -- Staff can add/edit attendee details on check-in or signup
...
create policy "Allow registrations write to all roles (Staff can register/check-in)"
  on public.registrations for all
  to authenticated
  using (true); -- Staff can register and check in attendees
...
create policy "Allow tickets write to all roles"
...
create policy "Allow notifications write to authenticated users"
